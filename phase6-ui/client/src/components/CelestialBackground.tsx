...
interface NebulaVol {
  ...
  rx: number; ry: number     // radii as fraction of width
...
interface CloudWisp {
  x0: number       // initial phase [0, 1] within one full cycle
  y: number        // Y center fraction [0, 1]
  w: number        // total wisp width as fraction of screen width
  h: number        // puff radius scale as fraction of screen width
  opacity: number  // peak puff opacity
  speed: number    // drift speed per t-unit (t += 0.008/frame)
  puffs: number    // number of overlapping elliptical blobs
  undulate: number // vertical undulation amplitude (screen height fraction)
  phase: number    // undulation phase offset
}

// Far, mid, and near cloud layers — each drifts left at different speeds for parallax depth.
// x0 staggers starting positions so clouds are distributed across the screen from the start.
// Speed formula: one full cycle (≈ period ≈ 1.6) at t+=0.008/frame
// Far layer ~90s: speed = 1.6/(90×0.48) ≈ 0.037
// Mid layer ~50s: speed = 1.6/(50×0.48) ≈ 0.067
// Near layer ~25s: speed = 1.6/(25×0.48) ≈ 0.133
const CLOUD_WISPS: CloudWisp[] = [
  ...
  // Near layer — large, faster, more diffuse (depth-of-field softness)
...
export default function CelestialBackground() {
  ...
  useEffect(() => {
    ...
    if (!canvas) return
      // Subscribe to speech energy — drives sky glow modulation
      const unsub = subscribeSpeechEnergy((e) => { speechEnergyRef.current = e })
    const ctx = canvas.getContext('2d')!
    const dpr = Math.max(1, Math.min(window.devicePixelRatio || 1, 2))
    const themeRef = { current: document.documentElement.getAttribute('data-theme') === 'dark' ? 'dark' : 'light' }
  ...
  const speechBoost = speechEnergyRef.current
      ...
      if (isDark) {
        ...
        sky.addColorStop(0, '#06162b')
        sky.addColorStop(0.5, '#031022')
        sky.addColorStop(1, '#010713')
        ctx.fillStyle = sky
        ctx.fillRect(0, 0, W, H)

        const bloom = ctx.createRadialGradient(W * 0.58, H * 0.06, H * 0.03, W * 0.5, H * 0.2, H * 0.7)
        bloom.addColorStop(0, 'rgba(126,182,244,0.14)')
        bloom.addColorStop(1, 'rgba(126,182,244,0)')
        ctx.fillStyle = bloom
        ctx.fillRect(0, 0, W, H)
      } else {
        // Grey heaven palette: dark cloud-bed with luminous center.
        const sky = ctx.createLinearGradient(0, 0, 0, H)
        sky.addColorStop(0.00, '#606972')
        sky.addColorStop(0.36, '#727c86')
        sky.addColorStop(0.70, '#929ca7')
        sky.addColorStop(1.00, '#a8b1bb')
        ctx.fillStyle = sky
        ctx.fillRect(0, 0, W, H)

        // Broad center glow, breathing subtly for a heavenly radiance.
        const heavenPulse = 0.88 + 0.12 * Math.sin(t * 0.22)
        // Speech energy brightens the heavenly core on syllables
        const speechGlow = 1 + speechBoost * 0.70
        const bloom = ctx.createRadialGradient(W * 0.50, H * 0.16, H * 0.02, W * 0.50, H * 0.34, H * 0.78)
        bloom.addColorStop(0,   `rgba(255,255,255,${Math.min(0.90, 0.48 * heavenPulse * speechGlow).toFixed(3)})`)
        bloom.addColorStop(0.35,`rgba(246,249,255,${Math.min(0.55, 0.23 * heavenPulse * speechGlow).toFixed(3)})`)
        bloom.addColorStop(1,   'rgba(255,255,255,0)')
        ctx.fillStyle = bloom
        ctx.fillRect(0, 0, W, H)

        // Vertical light column to create the central "heavenly shiny" shaft.
        const shaft = ctx.createLinearGradient(W * 0.30, 0, W * 0.70, 0)
        shaft.addColorStop(0.00, 'rgba(255,255,255,0)')
        shaft.addColorStop(0.38, `rgba(245,249,255,${Math.min(0.22, 0.06 * heavenPulse * speechGlow).toFixed(3)})`)
        shaft.addColorStop(0.50, `rgba(255,255,255,${Math.min(0.45, 0.14 * heavenPulse * speechGlow).toFixed(3)})`)
        shaft.addColorStop(0.62, `rgba(245,249,255,${Math.min(0.22, 0.06 * heavenPulse * speechGlow).toFixed(3)})`)
        shaft.addColorStop(1.00, 'rgba(255,255,255,0)')
        ctx.fillStyle = shaft
        ctx.fillRect(0, 0, W, H)

        // Soft aureole near upper center for sacred highlight depth.
        const aureole = ctx.createRadialGradient(W * 0.5, H * 0.12, H * 0.01, W * 0.5, H * 0.12, H * 0.18)
        aureole.addColorStop(0, `rgba(255,255,255,${Math.min(0.80, 0.34 * heavenPulse * speechGlow).toFixed(3)})`)
      ...
      if (isDark) {
        ...
        for (const neb of NIGHT_NEBULAE) {
          ...
          const rx = neb.rx * W
          const ry = neb.ry * W
          const op = Math.min(neb.opacity * breathe, 0.54)
          // Speech energy subtly brightens nebula on syllables
        ...
        for (const wisp of CLOUD_WISPS) {
          ...
          for (let i = 0; i < wisp.puffs; i++) {
            ...
            // Gentle vertical undulation gives wisps a drifting, organic feel
            const puffY  = wisp.y * H + wisp.undulate * H * Math.sin(t * 0.14 + wisp.phase + frac * Math.PI * 2.5)

            // Bell-curve profile: centre puffs are largest and most opaque
            const bell   = Math.sin(frac * Math.PI)
            const puffR  = wisp.h * W * (0.48 + 0.80 * bell)
            const puffOp = wisp.opacity * (0.38 + 0.62 * bell)
      ...
      for (const star of STARS) {
        ...
        const starScale = isDark ? 1.0 : 0.35
      ...
      // Soft atmospheric veil; day remains luminous, night gets slight depth.
      const veil = ctx.createLinearGradient(0, 0, 0, H)
      if (isDark) {
        veil.addColorStop(0, 'rgba(0,0,0,0.0)')
        veil.addColorStop(1, 'rgba(0,6,16,0.26)')
      } else {
        veil.addColorStop(0, 'rgba(255,255,255,0.0)')
        veil.addColorStop(1, 'rgba(72,80,92,0.22)')
      }
      ctx.fillStyle = veil
      ctx.fillRect(0, 0, W, H)
    }
