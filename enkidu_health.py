"""
enkidu_health.py — Unified health checker for all Enkidu subsystems.
...
Used by:
  - `python enkidu_check.py`           (startup self-test CLI)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Enkidu startup health check")
    ...
    if s["critical_failures"] > 0:
        print(f"  {colors['fail']}✗ {s['critical_failures']} critical failure(s) — Enkidu may not start correctly.{colors['reset']}\n")
        sys.exit(1)
